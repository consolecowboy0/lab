...
build a very simple model to allow the goose to navigate the obstacles
in the goose race. **lab** provides you with simple tools to get
started, as well as importing a variety of handy libraries.
...
You can see how we might use this to try to build a basic model to guide
the goose’s behavior in the goose\_race. Let’s go through one random
iteration first, so we can see how it might pan out.
...
We’ve built out a simple model for moving our goose at random, but we
are just building out a dataset. Soon, we will build a model and see if
the goose can improve based on many iterations of the goose running the
race.
...
Now we can use our result set to build a very simple model. Then we can
...
We can work with this for the example, but in practice we will likely
need more complex models for this particular task. It is not difficult
to reason out why that is so.
...
We can see our goose in the bottom left corner with all the obstacles.
How should we proceed? We will consult the model.
...
#>     0.068     0.167     0.216     0.108
```

So our super basic recommends that we move in to the upper right.
**lab** gives us a very easy way to do this use goose\_race.

``` r
gr <- move(gr, x = 1, y = 1)
```

**lab** also gives us a way to visualize our movements. Just simply plot
the goose\_race class.
